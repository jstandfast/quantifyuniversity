      ...
      <div class = "w3-container w3-card-4 w3-black w3-opacity-min" style="max-width: auto">
        ...
        <div class='content'>
            ...
            <?php
                ...
                $query="SELECT DISTINCT FacultyID FROM user NATURAL JOIN faculty NATURAL JOIN faculty_dept NATURAL JOIN department WHERE user.UserID=faculty.FacultyID AND $searchtype LIKE '$search'";
			    ...
			    if($count==0) {
        		    ...
        		    for ($counter=0;$row=mysqli_fetch_assoc($result);++$counter)
        		    {
                        ...
                        $facultyfname=$facultydata['FirstName'];
                        $facultylname=$facultydata['LastName'];
                        $facultytype=$facultydata['FacultyType'];
                        $facultydept=$facultydata['DeptName'];
                        
        		    	print("<tr>
            		    	    <td class = 'clean'><a class = 'clean' href = 'AdminFacultyInfo.php?UserID=$facultyid'>Select</a></td>
            		    	    <td>$facultyid</td>
            		    	    <td>$facultyfname</td>
            		    	    <td>$facultylname</td>
            		    	    <td>$facultytype</td>
