      ...
      <div class = "w3-container w3-card-4 w3-black w3-opacity-min" style="max-width: auto">
        ...
        <div class = "smalldiv">
            <?php
    		    ...
    		    $query = "SELECT BuildingID FROM building ORDER BY BuildingID DESC";
                ...
                for($counter=0;$row=mysqli_fetch_row($result);$counter++) {
                    ...
                    $splitlastid=str_split($lastid,1);
                    
                    $idnumber = $splitlastid[5] . $splitlastid[6];
                    
                    $testbuildingid=intval($idnumber);
        	        
        	        if($testbuildingid>$largestbuildingid) {
        	            
        	            $largestbuildingid=$testbuildingid;
        	        }
        	        
                    
                }
                
                $largestbuildingid = intval($largestbuildingid) + 1;
                
                $buildingid = "BUILD" . $largestbuildingid;

    	        
    	   ?>
    	   
    	    <form action='' method='post'>
    	        <p>New Building ID: <?php echo $buildingid; ?></p>
    	        <p><label for='buildingname'>Building Name: </label>
                    <input name='buildingname' type='text'></p>
                <p><label for='buildinguse'>Building Use</label>
    	    ...
    	    <?php
	            ...
	            if($_SERVER['REQUEST_METHOD'] == 'POST') {
                    ...
                    if(!empty($_POST['buildingname'])) {
                        ...
                        if(isset($_POST['buildinguse'])) {
                            
                            $buildingname=$_POST['buildingname'];
                            
                            $buildinguse=$_POST['buildinguse'];
                        
                            $query="INSERT INTO building (BuildingID,BuildingName,BuildingUse) VALUES ('$buildingid','$buildingname','$buildinguse');";
                        
                            if ($con->query($query) === TRUE) {
                                echo "<p>$buildingid added to buildings.";
                            } else {
                                echo "<p>Error: " . $sql . "<br>" . $con->error;
                            }
                            
                        } else {
                            echo "<p>Must enter building use.</p>";
                        }
                        
                    } else {
                        echo "<p>Must enter building name.</p>";
                    }
